-- Fix claim_gift function to allow hearts to exceed the limit of 5 when gifted
...
BEGIN
    ...
    ELSIF v_gift.type = 'hearts' THEN
        -- Allow hearts to exceed the limit of 5 when gifted
        UPDATE profiles SET current_hearts = COALESCE(current_hearts, 0) + v_gift.amount WHERE id = v_user_id;
    ...
    ELSIF v_gift.type = 'avatar' THEN
        -- Add avatar to owned_avatars array only if not already owned
        -- We do this in a single UPDATE to avoid variable scope issues
        UPDATE profiles 
        SET owned_avatars = array_append(COALESCE(owned_avatars, '{}'), v_gift.item_id)
        WHERE id = v_user_id
        AND NOT (COALESCE(owned_avatars, '{}') @> ARRAY[v_gift.item_id]);
    END IF;

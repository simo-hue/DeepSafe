-- Update check constraint to include 'avatar'
ALTER TABLE gifts DROP CONSTRAINT gifts_type_check;
ALTER TABLE gifts ADD CONSTRAINT gifts_type_check CHECK (type IN ('credits', 'xp', 'item', 'hearts', 'avatar'));

-- Update claim_gift function to handle avatars
...
BEGIN
    ...
    ELSIF v_gift.type = 'avatar' THEN
        ...
        -- Check if already owned to avoid duplicates? array_append allows duplicates, but UI might handle it.
        -- Better to check.
        IF NOT (COALESCE(inventory, '{}') @> ARRAY[v_gift.item_id]) THEN
             UPDATE profiles 
